...
- **Ingesta activa (Vankom):** `vankom_scraper` por LMT API (solo lectura).
...
- `vankom_scraper` (sin puerto público; writer a Influx, read-only)
...
### Vankom scraper (read-only)

Implementado en `collector/vankom_scraper/collector.py`.

Secuencia de auth:
1. `GET /api/captcha`
2. `GET /api/captcha/verify?code=<captcha_code>`
3. `POST /api/login`

Con sesión autenticada lee (sin escritura de configuración):
- `GET /api/cn/online-users`
- `GET /api/alarm/list`
- `GET /api/alarm/list-history`
- opcional: `GET /api/param/status-info` (si defines `status_xpath`)
- opcional: `GET /api/param/value` para cada xpath en `param_xpaths`

Escribe en Influx:
- `vankom_summary`
- `vankom_online_user`
- `vankom_param_value` (si hay `param_xpaths`)
...
- dashboards:
  - `collector/grafana/provisioning/dashboards/baicell.json` (solo Baicell)
  - `collector/grafana/provisioning/dashboards/vankom.json` (solo Vankom)

Baicell incluye, entre otros:
...
Vankom incluye, entre otros:
- Estado y contadores (`online users`, alarmas real-time e histórico, cell status)
- Series de `SINR/RSRP` y `DL/UL MBR`
- Tabla `vankom_config` con parámetros de red leídos por API (ej. `arfcn_dl`, `arfcn_ul`)
...
- Vankom read-only: `VANKOM_TARGETS_JSON`, `VANKOM_SCRAPE_INTERVAL`
